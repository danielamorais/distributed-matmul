...
# Package Dana WASM Worker with file_packager
# Creates file_system.js containing all components

set -e

echo "=== Packaging Dana WASM Worker ==="
...
if [ ! -d "wasm_output" ]; then
    ...
    echo "Run ./compile-worker-wasm.sh first."
    exit 1
fi
...
echo "  2. Open http://localhost:8080/worker-dana-wasm.html"
echo "  3. Open http://localhost:8080/xdana.html to submit tasks"
