...
data_ordering = "Test instant (request order)"
data_seconds = "Test instant (sencods)"
...
stats_history[data_seconds] = stats_history[data_ordering]
stats_history_serial[data_seconds] = stats_history_serial[data_ordering]
sns.lineplot(data=stats_history, x=data_seconds, y='Requests/s', label=sds_label)
sns.lineplot(data=stats_history_serial, x=data_seconds, y='Requests/s', label=local_label)
...
requests_media = requests.groupby('Timestamp (seconds)')['Response Time (ms)'].mean().reset_index()
requests_media['Timestamp (seconds)'] = requests_media['Timestamp (seconds)'] - 16
requests_serial_media = requests_serial.groupby('Timestamp (seconds)')['Response Time (ms)'].mean().reset_index()
requests_serial_media['Timestamp (seconds)'] = requests_serial_media['Timestamp (seconds)'] - 16

sns.lineplot(data=requests, x='Timestamp (seconds)', y='Response Time (ms)', label=sds_label)
sns.lineplot(data=requests_serial, x='Timestamp (seconds)', y='Response Time (ms)', label=local_label)
